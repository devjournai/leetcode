...
var reverseEvenLengthGroups = function (head) {
    ...
    function performReversal(segmentHeadOriginal, lengthToReverse) {
        ...
        for (let loopIndexReverse = 0; loopIndexReverse < lengthToReverse; loopIndexReverse++) {
    ...
    while (groupPrecursor.next) {
        ...
        while (currentGroupTraversalNode && nodesFoundInGroup < groupExpectedLength) {
        ...
        if (actualGroupLengthFound % 2 === 0) {
            let newlyReversedHead = performReversal(firstNodeOfGroup, actualGroupLengthFound);
            groupPrecursor.next = newlyReversedHead;
        }

        for (let incrementer = 0; incrementer < actualGroupLengthFound; incrementer++) {
