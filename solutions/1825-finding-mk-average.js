class MinPQ {
    ...
    hasLeftChildOfElement(indexCheck) {
        return this.getLeftChildIndexOfElement(indexCheck) < this.heapContent.length;
    }

    hasRightChildOfElement(indexCheck) {
        return this.getRightChildIndexOfElement(indexCheck) < this.heapContent.length;
    }
    ...
    swapElements(indexOneParam, indexTwoParam) {
        [this.heapContent[indexOneParam], this.heapContent[indexTwoParam]] = [this.heapContent[indexTwoParam], this.heapContent[indexOneParam]];
    }
    ...
    bubbleUpElement() {
        ...
        while (this.hasParentOfElement(currentIndex) && this.getParentElement(currentIndex) > this.heapContent[currentIndex]) {
            this.swapElements(this.getParentIndexOfElement(currentIndex), currentIndex);
    ...
    bubbleDownElement() {
        ...
        while (this.hasLeftChildOfElement(elementIndex)) {
            ...
            if (this.hasRightChildOfElement(elementIndex) && this.getRightChildElement(elementIndex) < this.getLeftChildElement(elementIndex)) {
                smallestChildLocation = this.getRightChildIndexOfElement(elementIndex);
            }

            if (this.heapContent[elementIndex] < this.heapContent[smallestChildLocation]) {
                break;
            } else {
...
class MaxPQ {
    ...
    exchangeElements(idxOne, idxTwo) {
        [this.heapData[idxOne], this.heapData[idxTwo]] = [this.heapData[idxTwo], this.heapData[idxOne]];
    }
    ...
    siftUp() {
        ...
        while (this.hasSuperNode(currentPos) && this.getSuperNode(currentPos) < this.heapData[currentPos]) {
    ...
    siftDown() {
        ...
        while (this.hasLowerLeftChild(currentPos)) {
            ...
            if (this.hasLowerRightChild(currentPos) && this.getLowerRightChild(currentPos) > this.getLowerLeftChild(currentPos)) {
...
MKAverage.prototype.purgeElementsFromHeap = function(targetHeap) {
    while (!targetHeap.isHeapEmpty() && this.removedElementTracker.has(targetHeap.peekTop())) {
...
MKAverage.prototype.transferValue = function(sourceHeap, destinationHeap, isSourceMiddle, isDestinationMiddle) {
...
MKAverage.prototype.balanceAllPartitions = function() {
    ...
    while (hasChanged) {
        ...
        if (!this.smallestKPartition.isHeapEmpty() && !this.middleLowerPartition.isHeapEmpty() && 
            this.smallestKPartition.peekTop() > this.middleLowerPartition.peekTop()) {
            this.transferValue(this.smallestKPartition, this.middleLowerPartition, false, true);
            this.transferValue(this.middleLowerPartition, this.smallestKPartition, true, false);
            hasChanged = true;
        }

        // Ensure relative order: middleLowerPartition.peek() <= middleUpperPartition.peek()
        if (!this.middleLowerPartition.isHeapEmpty() && !this.middleUpperPartition.isHeapEmpty() && 
            this.middleLowerPartition.peekTop() > this.middleUpperPartition.peekTop()) {
            this.transferValue(this.middleLowerPartition, this.middleUpperPartition, true, true);
            this.transferValue(this.middleUpperPartition, this.middleLowerPartition, true, true);
            hasChanged = true;
        }

        // Ensure relative order: middleUpperPartition.peek() <= largestKPartition.peek()
        if (!this.middleUpperPartition.isHeapEmpty() && !this.largestKPartition.isHeapEmpty() && 
            this.middleUpperPartition.peekTop() > this.largestKPartition.peekTop()) {
            this.transferValue(this.middleUpperPartition, this.largestKPartition, true, false);
            this.transferValue(this.largestKPartition, this.middleUpperPartition, false, true);
            hasChanged = true;
        }
        ...
        while (this.smallestKPartition.currentSize() < this.kDiscardCount && !this.middleLowerPartition.isHeapEmpty()) {
            this.transferValue(this.middleLowerPartition, this.smallestKPartition, true, false);
            hasChanged = true;
        }
        ...
        while (this.largestKPartition.currentSize() < this.kDiscardCount && !this.middleUpperPartition.isHeapEmpty()) {
            this.transferValue(this.middleUpperPartition, this.largestKPartition, true, false);
            hasChanged = true;
        }
        ...
        const currentMiddleTotalSize = this.middleLowerPartition.currentSize() + this.middleUpperPartition.currentSize();

        while (currentMiddleTotalSize < targetMiddleTotalSize && !this.smallestKPartition.isHeapEmpty()) {
             this.transferValue(this.smallestKPartition, this.middleLowerPartition, false, true);
             hasChanged = true;
        }
        while (currentMiddleTotalSize < targetMiddleTotalSize && !this.largestKPartition.isHeapEmpty()) {
             this.transferValue(this.largestKPartition, this.middleUpperPartition, false, true);
             hasChanged = true;
        }
        while (currentMiddleTotalSize > targetMiddleTotalSize && !this.middleUpperPartition.isHeapEmpty()) {
            // If middle is too large, move the largest from middle to largeKPartition
            this.transferValue(this.middleUpperPartition, this.largestKPartition, true, false);
            hasChanged = true;
        }
        while (currentMiddleTotalSize > targetMiddleTotalSize && !this.middleLowerPartition.isHeapEmpty()) {
        ...
        while (this.middleLowerPartition.currentSize() > this.middleUpperPartition.currentSize() + 1 && !this.middleLowerPartition.isHeapEmpty()) {
            this.transferValue(this.middleLowerPartition, this.middleUpperPartition, true, true);
            hasChanged = true;
        }
        while (this.middleUpperPartition.currentSize() > this.middleLowerPartition.currentSize() + 1 && !this.middleUpperPartition.isHeapEmpty()) {
            this.transferValue(this.middleUpperPartition, this.middleLowerPartition, true, true);
            hasChanged = true;
        }
    }
...
MKAverage.prototype.addElement = function(num) {
    ...
    if (this.elementsInStream.length > this.mWindowSize) {
        ...
        this.removedElementTracker.set(oldestElementValue, (this.removedElementTracker.get(oldestElementValue) || 0) + 1);
    }
...
MKAverage.prototype.calculateMKAverage = function() {
    ...
    const targetMiddleTotalSizeForCalculation = this.mWindowSize - (2 * this.kDiscardCount);

    // Rebalance one last time to ensure accurate state before calculation
    this.balanceAllPartitions(); 

    if (this.middleLowerPartition.currentSize() + this.middleUpperPartition.currentSize() !== targetMiddleTotalSizeForCalculation) {
        ...
        return Math.floor(this.currentMiddleSumValue / targetMiddleTotalSizeForCalculation);
    }
