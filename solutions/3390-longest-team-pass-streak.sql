...
-- Intuition: A team's streak is consecutive successful same-team passes in timestamp order; a turnover resets the streak to 0. Report each team that has a positive max streak.
-- Approach: Join passer/receiver teams, rank each team's outgoing passes by time, then recursively add 1 on a successful pass or reset to 0. MAX(streak) per team, drop zeros.
