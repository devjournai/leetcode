...
var friendRequests = function (
  ...
  const parentArray = Array.from(
    { length: totalPeople },
    (_, indexIdentifier) => indexIdentifier
  );
  ...
  const findSetRepresentative = (memberIdentifier) => {
    ...
    parentArray[memberIdentifier] = findSetRepresentative(
      parentArray[memberIdentifier]
    );
  ...
  for (
    ...
    requestCounter < friendRequestsList.length;
    ++requestCounter
  ) {
    ...
    for (
      let restrictionIterator = 0;
      restrictionIterator < restrictedPairs.length;
      ++restrictionIterator
    ) {
      const [forbiddenPersonX, forbiddenPersonY] =
        restrictedPairs[restrictionIterator];
      const rootForbiddenX = findSetRepresentative(forbiddenPersonX);
      const rootForbiddenY = findSetRepresentative(forbiddenPersonY);

      if (
        (rootOfPersonOne === rootForbiddenX &&
          rootOfPersonTwo === rootForbiddenY) ||
        (rootOfPersonOne === rootForbiddenY &&
          rootOfPersonTwo === rootForbiddenX)
      ) {
