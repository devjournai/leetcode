...
var possiblyEquals = function (s1, s2) {
    ...
    const dynamicProgrammingTable = new Array(firstString.length + 1).fill(null).map(() =>
        new Array(secondString.length + 1).fill(null).map(() => ({}))
    );
    ...
    function checkPossibility(idxOne, idxTwo, lengthDiff) {
        ...
        if (idxOne < firstString.length && idxTwo < secondString.length &&
            currentCharAtOne === currentCharAtTwo && lengthDiff === 0 &&
        ...
        if (idxOne < firstString.length && isNaN(currentCharAtOne) && lengthDiff < 0) {
            if (checkPossibility(idxOne + 1, idxTwo, lengthDiff + 1)) {
                return dynamicProgrammingTable[idxOne][idxTwo][lengthDiff] = true;
            }
        }

        if (idxTwo < secondString.length && isNaN(currentCharAtTwo) && lengthDiff > 0) {
        ...
        for (let digitAdvanceOne = 0; digitAdvanceOne < 3 && idxOne + digitAdvanceOne < firstString.length; digitAdvanceOne++) {
            const digitCharOne = firstString[idxOne + digitAdvanceOne];
            if (isNaN(digitCharOne)) {
                break;
            }
            parsedNumberFromOne = parsedNumberFromOne * 10 + parseInt(digitCharOne, 10);
            if (checkPossibility(idxOne + digitAdvanceOne + 1, idxTwo, lengthDiff + parsedNumberFromOne)) {
                return dynamicProgrammingTable[idxOne][idxTwo][lengthDiff] = true;
            }
        }

        let parsedNumberFromTwo = 0;
        for (let digitAdvanceTwo = 0; digitAdvanceTwo < 3 && idxTwo + digitAdvanceTwo < secondString.length; digitAdvanceTwo++) {
            const digitCharTwo = secondString[idxTwo + digitAdvanceTwo];
            if (isNaN(digitCharTwo)) {
                break;
            }
            parsedNumberFromTwo = parsedNumberFromTwo * 10 + parseInt(digitCharTwo, 10);
            if (checkPossibility(idxOne, idxTwo + digitAdvanceTwo + 1, lengthDiff - parsedNumberFromTwo)) {
                return dynamicProgrammingTable[idxOne][idxTwo][lengthDiff] = true;
            }
        }
