...
var isScramble = function (s1, s2) {
    ...
    const memoizationGrid = new Array(stringLength)
        .fill(null)
        .map(() => new Array(stringLength)
            .fill(null)
            .map(() => new Array(stringLength).fill(null)));

    function checkCharacterCounts(sourceString, sourceStart, sourceEnd, targetString, targetStart, targetEnd) {
        const characterFrequencies = new Array(26).fill(0);
        for (let idxOne = sourceStart; idxOne <= sourceEnd; idxOne++) {
            characterFrequencies[sourceString.charCodeAt(idxOne) - 'a'.charCodeAt(0)]++;
        }
        for (let idxTwo = targetStart; idxTwo <= targetEnd; idxTwo++) {
            characterFrequencies[targetString.charCodeAt(idxTwo) - 'a'.charCodeAt(0)]--;
        }
    ...
    function scrambleChecker(s1SubstrStart, s1SubstrEnd, s2SubstrStart) {
        ...
        if (currentSubstrLength === 1) {
            ...
            memoizationGrid[s1SubstrStart][s1SubstrEnd][s2SubstrStart] = singleCharMatch;
            return singleCharMatch;
        }

        if (!checkCharacterCounts(s1, s1SubstrStart, s1SubstrEnd, s2, s2SubstrStart, s2SubstrEnd)) {
        ...
        for (let partitionIndex = s1SubstrStart; partitionIndex < s1SubstrEnd; partitionIndex++) {
            const leftSegmentLength = partitionIndex - s1SubstrStart + 1;
            const rightSegmentLength = currentSubstrLength - leftSegmentLength;

            const noSwapOption = scrambleChecker(s1SubstrStart, partitionIndex, s2SubstrStart) &&
                scrambleChecker(partitionIndex + 1, s1SubstrEnd, s2SubstrStart + leftSegmentLength);

            const swapOption = scrambleChecker(s1SubstrStart, partitionIndex, s2SubstrStart + rightSegmentLength) &&
        ...
        memoizationGrid[s1SubstrStart][s1SubstrEnd][s2SubstrStart] = possibleScramble;
