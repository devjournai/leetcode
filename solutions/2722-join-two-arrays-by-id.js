/**
    ...
    * Time Complexity: O((N + M) * K + U log U), where N is the length of arr1, M is the length of arr2, K is the average number of properties in an object (for object spreading operations), and U is the number of unique IDs (U <= N + M).
    * Space Complexity: O(U * K), for storing up to U unique merged objects in the map and the final array, where each object can have up to K properties. In the worst case, this is O((N + M) * K).
*/
var join = function(arr1, arr2) {
    ...
    const finalSortedArray = mapValuesArray.sort((recordA, recordB) => recordA.id - recordB.id);
